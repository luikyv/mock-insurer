...
ignored_modules = [
    ...
    # test will always fail as it needs user intervention during its execution
    "consents_api_revoked-aspsp_test-module_v3-2",
    # test is currently unable to run against MB
    "automatic-payments_api_automatic-pix-consent-edition-account-holder_test-module_v2n2",
    # tests are no longer required to be executed in the upload job
    "exchange_api_operational-limits_test-module",
    "bank-fixed-incomes_api_operational-limits_test-module_v1",
    "credit-fixed-incomes_api_operational-limits_test-module_v1",
    "funds_api_operational-limits_test-module",
    "treasure-title_api_operational-limits_test-module",
    "variable-incomes_api_operational-limits_test-module_v1-2-1",
    "consents_api_operational-limits_test-module_v3-2",
    "resources_api_operational-limits_test-module_v3",
    "accounts_api_operational-limits_test-module_v2-4",
    "invoice-financings_api_operational-limits_test-module_v2-3",
    "loans_api_operational-limits_test-module_v2-5",
    "financings_api_operational-limits_test-module_v2-3",
    "unarranged-accounts-overdraft_api_operational-limits_test-module_V2-4",
    "credit-cards_api_operational-limits_test-module_v2-1",

    # This script doesn't work properly when the test finishes with status "INTERRUPTED", this is mainly a problem in the conformance suite itself.
    # We'll ignore it for now, but we should fix it in the future.
    "accounts_api_multiple-consents_test-module_v2-4", # Multiple consents tests requires user intervention.
]
...
def analyze_result_logs(module_id, test_name, variant, test_result, plan_result, logs, expected_failures_list, expected_skips_list, counts_unexpected):
    ...
    for log_entry in logs:
        ...
        if (
            log_entry['src'] == 'WebRunner' or (log_entry['src'] == test_name and 'Web Runner Exception' in log_entry['msg'])
        ) and log_result == 'FAILURE':
...
async def main():
    ...
    for result in results:
        print('\n\n\n\n --- RESULTS FOR {}  ---\n with configuration {}:'.format(result['test_plan'].capitalize(), result['config_file']))
